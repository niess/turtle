...
int main()
{
/** First let us initialise the TURTLE library. The `NULL` argument specifies
 * that no error handler is used. Most of TURTLE's library functions return
 * a `turtle_return` code that could be catched by a user defined error handler.
 * But, to start with let us handle explicitly the return codes.
 */
	/* Initialise the TURTLE library. */
	turtle_initialise(NULL);

/** Let us now demonstrate how to load a projection map from the disk and how
 * to access its meta-data. So you'll need to run `example-projection` first if
...
/** For the following, let us attach an error handler to TURTLE such that we
 ...
 */
	...
	turtle_projection_unproject(rgf93, box.x0, box.y0,
		&latitude, &longitude);
	...
	double xUTM, yUTM;
	turtle_projection_project(utm, latitude, longitude,
		&xUTM, &yUTM);
...
/** Let us show how to access the ASTER-GDEM2 elevation data. That for we need
 * to instanciate a geodetic `turtle_datum`. Since we are only interested in a
 * single coordinate the stack size for elevation data is set to `1`, i.e. a
 * single data file will be loaded and buffered.
 *
 * Having a new `turtle_datum` for ASTER-GDEM2 data, we can request the origin's
 * elevation and compare the result to the map's one. *Note* that there might be
 * a 1 cm difference between both due to the encoding of the elevation data over
 * 16 bits.
 */
	/*
...
/** Finally, let us express the origin's coordinates in a Cartesian frame,
 * i.e. Earth-Centered, Earth-Fixed (ECEF). *Note* that this conversion doesn't
 * require access to the ASTER-GDEM2 elevation data, provided that the elevation
 * is known from elsewhere. E.g. in this case we use the map's elevation value.
 */
	double ecef[3];
